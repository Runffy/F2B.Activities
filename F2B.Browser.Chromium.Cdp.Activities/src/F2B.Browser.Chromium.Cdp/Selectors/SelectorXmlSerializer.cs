...
namespace F2B.Browser.Chromium.Cdp.Selectors
{
    ...
    public static class SelectorXmlSerializer
    {
        ...
        /// <summary>
        /// Splits selector scope for tab/element operations. Rejects <c>wnd</c> and invalid level tags.
        /// </summary>
        public static SelectorScope SplitScopeForOperation(string selectorXml)
        {
            var scope = SplitScope(selectorXml);
            if (scope.RequiresTabResolution)
            {
                throw new InvalidOperationException("Selector must not contain <wnd> when searching within a tab or element.");
            }

            ValidateOperationLevels(scope.FrameLevels);
            ValidateOperationLevels(scope.ElementLevels);
            return scope;
        }

        private static void ValidateOperationLevels(IList<SelectorLevel> levels)
        {
            if (levels == null)
            {
                return;
            ...
            foreach (var level in levels)
            {
                ...
                var tag = level.TagName ?? string.Empty;
                if (!string.Equals(tag, "frm", StringComparison.OrdinalIgnoreCase) &&
                    !string.Equals(tag, "ctrl", StringComparison.OrdinalIgnoreCase) &&
                    !string.Equals(tag, "parent", StringComparison.OrdinalIgnoreCase))
                {
                    throw new InvalidOperationException(
                        string.Format("Unsupported selector level <{0}>. Only frm, ctrl and parent are allowed.", tag));
                }
            }
        }
        ...
        private static void EnsureMinimumParentAttributes(SelectorLevel level, IList<string> attrs)
        {
            ...
            var value = levelProperty != null ? levelProperty.Value : "1";
        ...
        private static string ResolveWireAttributeName(string tagName, string propertyName)
        {
            ...
            string mapped;
            if (AttributeMap.TryGetValue(propertyName, out mapped))
            {
                return mapped;
            }
