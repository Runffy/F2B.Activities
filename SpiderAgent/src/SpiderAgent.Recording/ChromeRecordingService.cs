...
public sealed class ChromeRecordingService : IAsyncDisposable
{
    ...
    public event EventHandler<RequestCapturedEventArgs>? RequestCaptured;
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (!_bridgeServer.IsExtensionConnected)
        {
            throw new InvalidOperationException(
                "Chrome 扩展尚未连接。请先打开 Chrome 并确认 SpiderAgent 扩展已加载且状态正常。");
        }
    ...
    private void HandleNetworkEvent(BridgeMessage message)
    {
        ...
        var captured = _currentSession.Requests[^1];
        RequestCaptured?.Invoke(this, new RequestCapturedEventArgs
        {
            Request = captured,
            TotalCount = _currentSession.Requests.Count
        });
    }
