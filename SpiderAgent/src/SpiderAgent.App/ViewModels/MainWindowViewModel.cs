...
public sealed class MainWindowViewModel : ObservableObject
{
    ...
    private string _unifiedPrompt = string.Empty;
    private bool _isSplitPromptMode = true;
        ...
        SessionTitleGenerator sessionTitleGenerator)
    {
        ...
        AnalyzeOrStopCommand = new AsyncRelayCommand(OnAnalyzeOrStopAsync, () => CanAnalyzeOrStop);
        ...
        _recordingService.RequestCountChanged += (_, count) =>
            RunOnUiThread(() => RecordedRequestCount = count, DispatcherPriority.DataBind);
        _recordingService.RequestCaptured += (_, args) =>
            RunOnUiThread(() => OnRequestCaptured(args), DispatcherPriority.DataBind);
    ...
    /// <summary>
    /// 首次分析完成后使用的单一 Prompt 输入框（继续优化 / 修复要求）。
    /// </summary>
    public string UnifiedPrompt
    {
        get => _unifiedPrompt;
        set
        {
            if (SetProperty(ref _unifiedPrompt, value))
            {
                RaiseCommandStates();
            }
        }
    ...
    /// <summary>
    /// 首次分析前：分「操作步骤」与「期望目标」两个输入框。
    /// </summary>
    public bool IsSplitPromptMode
    {
        get => _isSplitPromptMode;
        private set
        {
            if (SetProperty(ref _isSplitPromptMode, value))
            {
                OnPropertyChanged(nameof(IsRefinementPromptMode));
            }
        }
    }

    public bool IsRefinementPromptMode => !IsSplitPromptMode;

    private int _maxSelfTestAttempts = 3;
    private int _selfTestTimeoutSeconds;
    ...
    /// <summary>
    /// 单次自测脚本最长运行秒数。0 表示不限制（批量下载等长任务推荐）。
    /// </summary>
    public int SelfTestTimeoutSeconds
    {
        get => _selfTestTimeoutSeconds;
        set
        {
            if (value < 0) value = 0;
            SetProperty(ref _selfTestTimeoutSeconds, value);
        }
    }
    ...
    private bool HasValidCombinedPrompt()
        => IsSplitPromptMode
            ...
            : !string.IsNullOrWhiteSpace(UnifiedPrompt);

    private string BuildCombinedUserPrompt()
    {
        if (IsSplitPromptMode)
        {
    ...
    private void ClearPromptFields()
    {
        ...
        UnifiedPrompt = string.Empty;
        IsSplitPromptMode = true;
    }

    private void EnterRefinementPromptMode(bool clearUnified = true)
    {
        IsSplitPromptMode = false;
        if (clearUnified)
        {
            UnifiedPrompt = string.Empty;
        }
    ...
    private void ApplyPromptModeForSession(WorkspaceSessionMetadata? metadata, IChatSession? analysisSession)
    {
        var hasPriorAnalysis = analysisSession is { History.Count: > 0 }
                               || (metadata?.HasAnalysisHistory ?? false);

        if (hasPriorAnalysis)
        {
            EnterRefinementPromptMode(clearUnified: true);
        }
        else
        {
            IsSplitPromptMode = true;
    ...
    public AsyncRelayCommand AnalyzeOrStopCommand { get; }
    ...
    private async Task OnAnalyzeOrStopAsync()
    {
        if (IsAnalyzing)
        {
    ...
    private async Task StartRecordingAsync()
    {
        try
        {
            if (!EnsureExtensionConnectedForRecording())
            {
                return;
            }
    ...
    private bool EnsureExtensionConnectedForRecording()
    {
        if (_recordingService.IsBridgeConnected)
        {
            return true;
        }

        MessageBox.Show(
            "Chrome 扩展尚未连接，无法开始录制。\n\n" +
            "请确认：\n" +
            "1. 已打开 Google Chrome\n" +
            "2. 已在 chrome://extensions 加载 SpiderAgent 扩展\n" +
            "3. 扩展已启用且未被禁用\n\n" +
            "完成上述步骤后，请再次点击「开始录制」。",
            "无法开始录制",
            MessageBoxButton.OK,
        ...
        AppendOutput("[系统] 未能开始录制：扩展尚未连接。请确认 Chrome 与扩展正常后重试。");
        return false;
    }

    private void OnRequestCaptured(RequestCapturedEventArgs args)
    {
        if (RecordingStatus != RecordingStatus.Recording)
        {
            return;
        ...
        RecordedRequestCount = args.TotalCount;

        var request = args.Request;
        var status = request.StatusCode?.ToString() ?? "-";
        var type = string.IsNullOrWhiteSpace(request.ResourceType) ? "?" : request.ResourceType;
        var url = request.Url.Length <= 220 ? request.Url : request.Url[..220] + "…";
        AppendOutput($"[录制] [{args.TotalCount}] {request.Method} {status} {type} {url}");
    }
    ...
    private async Task RunAnalyzeAsync()
    {
        ...
        try
        {
            ...
            if (!isRefinement)
            {
                var systemPrompt = await BuildSystemPromptInBackgroundAsync(session, prompt, cancellationToken);
                ...
                await StreamAnalysisAsync(
                    ...
                    _streamBuffer,
                    cancellationToken,
                    useStreamLock: true).ConfigureAwait(false);
            }
            else
            {
                // 继续优化 / 迭代：复用已有聊天上下文，直接发新 prompt 作为跟进指令
                string? existingScriptFileName = null;
                if (_currentWorkspaceSessionId is not null)
                {
                    ...
                    {
                        existingScriptFileName = Path.GetFileName(metadata.LastOutputScriptPath);
                    }
                }
                ...
                var targetFileHint = string.IsNullOrWhiteSpace(existingScriptFileName)
                    ? "上一次生成的 .py 文件"
                    : $"`{existingScriptFileName}`";
                ...
                string refinementUserMessage =
                    ...
                    "要求：\n" +
                    $"- **必须在原文件 {targetFileHint} 上修改**，输出完整可运行脚本；**禁止**更换文件名或新建另一个 .py。\n" +
                    "- **不要输出** ```filename 块。\n" +
                    "- 保留之前正确、已验证可运行的部分，只针对新要求做必要修改或增强。\n" +
                    "- 继续严格遵守系统Prompt中的所有硬性规则（包括对「录制快照固定值（来源未知）」的注释要求、多请求分函数封装、自测信号打印等）。\n" +
                    "- 直接输出 ```text requirements 和 ```python 脚本块，不要重复整篇分析报告。\n" +
                    "- 仅当业务目标真正完成时打印 `SUCCESS:` 信号；任何步骤失败须 `raise` 或 `sys.exit(1)`，禁止 print 错误后继续。";
        ...
        finally
        {
            ...
            {
                IsAnalyzing = false;
                if (_analysisSession is { History.Count: > 0 })
                {
                    EnterRefinementPromptMode(clearUnified: true);
                }
            });
    ...
    private async Task<string> BuildSystemPromptInBackgroundAsync(
        RecordingSession session,
        string prompt,
        CancellationToken cancellationToken)
    {
        AppendOutput($"[系统] 正在准备分析上下文（共 {session.Requests.Count} 条录制请求，请稍候）…");

        return await Task.Run(
            () => _analysisContextBuilder.BuildSystemPrompt(session, prompt),
            cancellationToken).ConfigureAwait(false);
    }

    private void BeginAgentStreamingWaitBlock()
    {
        _outputLog.BeginStreamingBlock();
        _outputLog.UpdateStreamingBlock("[Agent] （已发送请求，等待首次回复…）", immediate: true);
    }
    ...
    private static string ExtractPythonCode(string content)
    {
        ...
        if (startIndex >= 0)
        {
            ...
            if (endIndex > startIndex)
            {
                ...
                return LooksLikePythonCode(fromFence) ? fromFence : string.Empty;
            }
        }

        var trimmed = content.Trim();
        return LooksLikePythonCode(trimmed) ? trimmed : string.Empty;
    }

    private static bool LooksLikePythonCode(string code)
    {
        ...
        return Regex.IsMatch(
                   head,
                   @"(?m)^\s*(#|import |from .+ import |def |class |if __name__)",
                   RegexOptions.IgnoreCase)
               || head.Contains("requests.", StringComparison.Ordinal)
               || head.Contains("subprocess.", StringComparison.Ordinal);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var overwriteExisting = !string.IsNullOrWhiteSpace(preferredScriptFileName);
        var finalFileName = AgentScriptFileNameResolver.Resolve(
            extractedFileName,
            preferredScriptFileName,
            overwriteExistingFileName: overwriteExisting);
        if (overwriteExisting)
        {
            if (!string.IsNullOrWhiteSpace(extractedFileName)
                && !string.Equals(
                    AgentScriptFileNameResolver.Resolve(extractedFileName, null),
                    finalFileName,
                    StringComparison.OrdinalIgnoreCase))
            {
                AppendOutput($"[系统] 继续优化模式：忽略 Agent 新文件名，覆盖原文件: {finalFileName}");
            }
            else
            {
                AppendOutput($"[系统] 继续优化模式：覆盖原脚本文件: {finalFileName}");
            }
        }
        else if (string.IsNullOrWhiteSpace(extractedFileName))
        {
        ...
        while (true)
        {
            ...
            var result = await _pythonValidator.ValidateAsync(
                ...
                new PythonValidationOptions
                {
                    ...
                    TimeoutMs = SelfTestTimeoutSeconds > 0
                        ? SelfTestTimeoutSeconds * 1000
                        : 0,
                    Log = AppendOutput
                },
                cancellationToken);
            ...
            if (result.Success)
            {
                ...
                AppendSelfTestOutputPreview(result);
    ...
    private static string BuildRepairPrompt(string previousFullResponse, PythonScriptValidator.ValidationResult failure)
    {
        ...
        sb.AppendLine("- 确保脚本在成功时能打印清晰的成功信号（如“SUCCESS: 共下载 N 集”“共找到 N 个...”），并正常退出（exit code 0）。");
        sb.AppendLine("- 环境/依赖缺失时必须 sys.exit(1)，禁止 print 错误后 return；未完成任务时不得打印 SUCCESS。");
        sb.AppendLine("- 不要重复整个逆向分析报告，只给可直接落地的修复版本。");

        return sb.ToString();
    }

    private void AppendSelfTestOutputPreview(PythonScriptValidator.ValidationResult result)
    {
        var combined = (result.StdOut + "\n" + result.StdErr).Trim();
        if (string.IsNullOrWhiteSpace(combined))
        {
            AppendOutput("[自测] 脚本无标准输出。");
            return;
        }

        const int maxChars = 1200;
        var preview = combined.Length <= maxChars
            ? combined
            : combined[^maxChars..];
        AppendOutput("[自测] 脚本输出摘要:");
        AppendOutput(preview);
    }
    ...
    private async Task SwitchToWorkspaceSessionAsync(string sessionId)
    {
        try
        {
            ...
            ClearPromptFields();
            ApplyPromptModeForSession(metadata, _analysisSession);
    ...
    {
        var label = IsRefinementPromptMode ? "[用户] Prompt：" : "[用户] 整合后的 Prompt：";
        AppendOutput(label);
        AppendOutput(combinedPrompt);
    }

    private static void RunOnUiThread(Action action, DispatcherPriority priority = DispatcherPriority.Background)
    {
