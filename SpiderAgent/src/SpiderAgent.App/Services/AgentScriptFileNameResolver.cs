...
public static class AgentScriptFileNameResolver
{
    ...
    /// <param name="overwriteExistingFileName">
    /// 继续优化时为 true：强制覆盖已有脚本文件，忽略 Agent 新输出的 filename。
    /// </param>
    public static string Resolve(
        string? rawName,
        string? fallbackRawName = null,
        bool overwriteExistingFileName = false)
    {
        if (overwriteExistingFileName && !string.IsNullOrWhiteSpace(fallbackRawName))
        {
            return EnsurePyExtension(NormalizeFileName(fallbackRawName));
        }
        ...
        return EnsurePyExtension(baseName);
    }

    private static string EnsurePyExtension(string baseName)
        => baseName.EndsWith(".py", StringComparison.OrdinalIgnoreCase)
            ? baseName
            : $"{baseName}.py";
