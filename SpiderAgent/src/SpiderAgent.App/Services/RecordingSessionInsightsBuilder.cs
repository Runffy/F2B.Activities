...
public sealed class RecordingSessionInsightsBuilder
{
    ...
    public string ToMarkdown(SessionInsights insights, IReadOnlyList<RecordedRequest> ordered)
    {
        ...
        builder.AppendLine();
        AppendHlsEncryptionSection(builder, ordered);

        builder.AppendLine();
        builder.AppendLine("### 15. 代码硬性约束");
    ...
    private static void AppendHlsEncryptionSection(StringBuilder builder, IReadOnlyList<RecordedRequest> ordered)
    {
        builder.AppendLine("### 14. HLS / m3u8 加密与二进制 Key 响应（必读）");

        var encryptionHints = RecordedResponseBodyFormatter.DetectHlsEncryption(ordered);
        var keyRequests = RecordedResponseBodyFormatter.FindLikelyHlsKeyRequests(ordered);

        if (encryptionHints.Count == 0 && keyRequests.Count == 0)
        {
            builder.AppendLine("- 未检测到明显的 HLS AES 加密 m3u8 或 key 请求。");
            return;
        }

        if (encryptionHints.Count > 0)
        {
            builder.AppendLine("- 检测到 **m3u8 含 EXT-X-KEY 加密**：");
            foreach (var hint in encryptionHints)
            {
                builder.AppendLine($"  - m3u8: {Truncate(hint.M3u8Url, 120)}");
                builder.AppendLine($"    METHOD={hint.Method}, KEY-URI={hint.KeyUri ?? "(内联)"}");
            }

            builder.AppendLine("- 生成代码时 **必须** 先请求 KEY-URI 获取 AES key（二进制），再解密 TS；**禁止**假设 m3u8 可直接用 ffmpeg 合并。");
        }

        if (keyRequests.Count > 0)
        {
            builder.AppendLine("- 检测到 **疑似 HLS AES key 的二进制响应**（录制中已 Base64 保存，勿当 UTF-8 文本解析）：");
            foreach (var request in keyRequests.Take(8))
            {
                var preview = RecordedResponseBodyFormatter.FormatBinaryPreview(request)
                    .Replace('\n', ' ')
                    .Trim();
                builder.AppendLine($"  - {Truncate(request.Url, 120)} — {preview}");
            }

            builder.AppendLine("- 在 Python 中应 `base64.b64decode(录制中的 Base64 字符串)` 或重新请求 key URL 获取 bytes；可配合 `m3u8`/`pycryptodome` 解密 TS。");
        }
    }
