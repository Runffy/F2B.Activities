...
namespace F2B.Browser.IExplore.Com
{
    ...
    public static class IeLauncher
    {
        ...
        /// <summary>Default Trident IE executable used by <see cref="StartViaDefaultIExploreExe"/>.</summary>
        public const string DefaultIExploreExePath = @"C:\Program Files\Internet Explorer\iexplore.exe";

        /// <summary>
        /// Start IE via <see cref="DefaultIExploreExePath"/>. Passes <paramref name="url"/> as the only argument when set;
        /// otherwise starts with no arguments. Does not wait for a window.
        /// </summary>
        public static void StartViaDefaultIExploreExe(string url, out string methodUsed)
        {
            if (!File.Exists(DefaultIExploreExePath))
            {
                throw new InvalidOperationException(
                    "Internet Explorer was not found at:\n  " + DefaultIExploreExePath + "\n\n" + GetEnableIeInstructions());
            }
            ...
            {
                FileName = DefaultIExploreExePath,
                Arguments = arguments ?? string.Empty,
                UseShellExecute = false
            });

            methodUsed = DefaultIExploreExePath;
        }
        ...
        /// <summary>Start IE visible via COM without navigation (matches ComHost <c>launch</c> with no URL).</summary>
        public static void StartVisibleOnly(out string methodUsed)
        {
            string method = null;
            StaInvoker.Invoke(() => StartVisibleOnlyCore(out method));
            methodUsed = method;
        }
        ...
        private static void StartVisibleOnlyCore(out string methodUsed)
        {
            methodUsed = null;
            if (Type.GetTypeFromProgID("InternetExplorer.Application") == null)
            {
        ...
        {
            var ie = CreateInternetExplorerApplication();
            ie.Visible = true;
            ie.Navigate2(url);
        }
        ...
        private static dynamic CreateInternetExplorerApplication()
        {
            var progId = Type.GetTypeFromProgID("InternetExplorer.Application");
            if (progId == null)
