...
{
    public sealed class BridgeElementTargetActivityDesigner : ActivityDesigner
    {
        ...
        public BridgeElementTargetActivityDesigner()
        {
            ...
            _elementExpressionBox = CreateExpressionTextBox("Element", typeof(BwElement));
            _elementRow = CreateRow("Element", _elementExpressionBox, out _elementEditorBorder, RowSpacing);
            _tabExpressionBox = CreateExpressionTextBox("InputTab", typeof(BwTab));
            ...
            _selectorRow = SelectorDesignerSupport.CreateSelectorRow("Selector", _selectorExpressionBox, "Selector", () => ModelItem, "ElementTargetLabelColumn", EditorMinWidth, out _selectorEditorBorder, RowSpacing);
            ...
            _childSelectorRow = SelectorDesignerSupport.CreateSelectorRow("ChildSelector", _childSelectorExpressionBox, "ChildSelector", () => ModelItem, "ElementTargetLabelColumn", EditorMinWidth, out _childSelectorEditorBorder, RowSpacing);
        ...
        private static ComboBox BuildValTypeComboBox()
        {
            ...
            comboBox.Items.Add(BridgeSelectValType.Text);
            comboBox.Items.Add(BridgeSelectValType.Value);
            comboBox.Items.Add(BridgeSelectValType.Index);
            return comboBox;
        }
        ...
        private static ComboBox BuildTargetTypeComboBox()
        {
            ...
            comboBox.Items.Add(BridgeElementTargetType.Selector);
            comboBox.Items.Add(BridgeElementTargetType.Element);
            return comboBox;
        }
        ...
        private void OnLoaded(object sender, RoutedEventArgs e)
        {
            ...
            if (_designerMode == DesignerMode.TakeScreenshot)
            {
                ...
                RefreshTakeScreenshotRows(baseOn, current);
            }
        ...
        private void OnTargetTypeSelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            if (_targetTypeComboBox.SelectedItem is BridgeElementTargetType targetType)
            {
                ...
                if (_designerMode == DesignerMode.TakeScreenshot)
                {
                    RefreshTakeScreenshotRows(ReadBridgeTakeScreenshotBaseOn(ModelItem), targetType);
                }
        ...
        private void OnBridgeTakeScreenshotBaseOnSelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            RefreshTakeScreenshotRows(baseOn, ReadTargetType(ModelItem));
            RefreshRequiredBorders();
        }
        ...
        private void OnValTypeSelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            if (_valTypeComboBox.SelectedItem is BridgeSelectValType valType)
            {
        ...
        private void SyncTargetTypeCombo(BridgeElementTargetType targetType)
        {
        ...
        private void SyncValTypeCombo(BridgeSelectValType valType)
        {
        ...
        private void RefreshTargetRows(BridgeElementTargetType targetType)
        {
            if (_designerMode == DesignerMode.TakeScreenshot &&
                ReadBridgeTakeScreenshotBaseOn(ModelItem) == BridgeTakeScreenshotBaseOn.Tab)
            {
                _elementRow.Visibility = Visibility.Collapsed;
                _tabRow.Visibility = Visibility.Visible;
                _selectorRow.Visibility = Visibility.Collapsed;
                return;
            }

            var isElement = targetType == BridgeElementTargetType.Element;
            var showTab = !isElement &&
            ...
            _tabRow.Visibility = showTab ? Visibility.Visible : Visibility.Collapsed;
        ...
        private void RefreshTakeScreenshotRows(BridgeTakeScreenshotBaseOn baseOn, BridgeElementTargetType targetType)
        {
            ...
            _takeScreenshotBaseOnRow.Visibility = Visibility.Visible;
            _pathRow.Visibility = Visibility.Visible;

            if (baseOn == BridgeTakeScreenshotBaseOn.Tab)
            {
                _targetTypeRow.Visibility = Visibility.Collapsed;
                _elementRow.Visibility = Visibility.Collapsed;
                _tabRow.Visibility = Visibility.Visible;
                _selectorRow.Visibility = Visibility.Collapsed;
                return;
            ...
            _targetTypeRow.Visibility = Visibility.Visible;
            RefreshTargetRows(targetType);
        }
        ...
        private void RefreshSelectRows(BridgeSelectValType valType)
        {
            if (_designerMode != DesignerMode.Select)
            {
                return;
            ...
            _textsRow.Visibility = valType == BridgeSelectValType.Text ? Visibility.Visible : Visibility.Collapsed;
            _valuesRow.Visibility = valType == BridgeSelectValType.Value ? Visibility.Visible : Visibility.Collapsed;
            _indicesRow.Visibility = valType == BridgeSelectValType.Index ? Visibility.Visible : Visibility.Collapsed;
        }
        ...
        private void OnModelItemPropertyChanged(object sender, PropertyChangedEventArgs e)
        {
            Dispatcher.BeginInvoke(new Action(() =>
            {
                ...
                if (string.Equals(e.PropertyName, "Selector", StringComparison.Ordinal))
                {
                    RefreshTargetRows(ReadTargetType(ModelItem));
                }
        ...
        private void RefreshRequiredBorders()
        {
            ...
            SetRequiredBorder(_textsEditorBorder, _designerMode == DesignerMode.Select && valType == BridgeSelectValType.Text, IsArgumentFilled(ModelItem, "Texts", _textsExpressionBox));
            SetRequiredBorder(_valuesEditorBorder, _designerMode == DesignerMode.Select && valType == BridgeSelectValType.Value, IsArgumentFilled(ModelItem, "Values", _valuesExpressionBox));
            SetRequiredBorder(_indicesEditorBorder, _designerMode == DesignerMode.Select && valType == BridgeSelectValType.Index, IsArgumentFilled(ModelItem, "Indices", _indicesExpressionBox));
        }
        ...
        private static BridgeElementTargetType ReadTargetType(ModelItem modelItem)
        {
        ...
        private static BridgeSelectValType ReadValType(ModelItem modelItem)
        {
            var modelProperty = modelItem.Properties["ValType"];
            if (modelProperty?.ComputedValue is BridgeSelectValType value)
            {
                return value;
            }

            return BridgeSelectValType.Text;
        }
        ...
        private void UpdateMetrics()
        {
            ...
            Debug.WriteLine("[BridgeElementTargetActivityDesigner] " + text);
        }
